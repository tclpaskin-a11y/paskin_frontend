              ...
              >
                {filteredOrders.length > 0 ? (
                  filteredOrders.map((order) => {
                    ...
                    const productImage = firstProduct.images?.[0] || "https://images.unsplash.com/photo-1611073103901-09605d8f6cc9?auto=format&fit=crop&q=80&w=300";
                    const orderDate = order.createdAt ? new Date(order.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "N/A";
                    ...
                    return (
                      <Card key={order._id} className="border-none shadow-soft hover:shadow-elegant transition-all group overflow-hidden">
                            ...
                            <div className="flex-1 p-5 md:p-6 space-y-4">
                              <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                                <div className="space-y-1 text-left">
                                  <p className="text-[10px] font-bold text-muted-foreground tracking-[0.2em] uppercase">{displayId}</p>
                                  <h3 className="text-base md:text-lg font-bold group-hover:text-primary transition-colors line-clamp-1">
                                    {productName}
                                    {order.products?.length > 1 && ` + ${order.products.length - 1} more`}
                                  </h3>
                                </div>
                                <Badge variant="outline" className={`px-4 py-1.5 rounded-full text-[10px] font-bold border-2 shrink-0 ${getStatusColor(orderStatus)}`}>
                                  {displayStatus}
                                </Badge>
                              </div>
                              ...
                              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 py-4 border-y border-slate-50 text-left">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <p className="text-xs md:text-sm font-medium capitalize">{order.paymentStatus || "Pending"}</p>
                                </div>
                                <div className="space-y-1 col-span-2">
                                  ...
                                  <p className="text-xs md:text-sm font-bold text-primary">₹{(order.totalAmount || 0).toLocaleString()}</p>
                                </div>
                              </div>
